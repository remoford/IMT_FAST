...
void convolv3waldpdf(double m1, double s1, double m2, double s2, double m3, double s3, const double X[], double Y[], int size_XY, double h)
{
	/*
	flag=0;
	*/
	...
	*/
	...
	s=abs(s);
	*/
	m[0] = fabs(m[0]); m[1] = fabs(m[1]); m[2] = fabs(m[2]);
	s[0] = fabs(s[0]); s[1] = fabs(s[1]); s[2] = fabs(s[2]);
	...
	*/
	...
	double m_copy[3]; double s_copy[3]; double v_copy[3];
	for (int i = 0; i < 3; i++) {
		m_copy[i] = m[i];
		s_copy[i] = s[i];
		v_copy[i] = v[i];
	...
	*/
	double T1 = (1.0 / m[0]) * (pow(1 + (9.0 / 4.0) * (pow(s[0], 4) / pow(m[0], 2)), 0.5) - (3.0 / 2.0)*(pow(s[0], 2) / m[0]));
	double T2 = (1.0 / m[1]) * (pow(1 + (9.0 / 4.0) * (pow(s[1], 4) / pow(m[1], 2)), 0.5) - (3.0 / 2.0)*(pow(s[1], 2) / m[1]));
	double T3 = (1.0 / m[2]) * (pow(1 + (9.0 / 4.0) * (pow(s[2], 4) / pow(m[2], 2)), 0.5) - (3.0 / 2.0)*(pow(s[2], 2) / m[2]));
	...
	*/
	if (sd[0] < 0.01) {
		...
		*/
		...
		double * check1_X1 = malloc(partitionLength1 * sizeof(double));
		double * check1_X2 = malloc(partitionLength2 * sizeof(double));
		double * check1_Y1 = malloc(partitionLength1 * sizeof(double));
		double * check1_Y2 = malloc(partitionLength2 * sizeof(double));
		double tally = 0;
		...
		for (int i = 0; i < partitionLength2; i++)
			check1 += check1_Y2[i];
		check1 = check1 * hh;
		free(check1_X1); free(check1_Y1);
		free(check1_X2); free(check1_Y2);
		...
		*/
		while (checkerror > 0.0001) {
			...
			*/
			...
			double * ck1_X1 = malloc(partitionLength1 * sizeof(double));
			double * ck1_X2 = malloc(partitionLength2 * sizeof(double));
			double * ck1_Y1 = malloc(partitionLength1 * sizeof(double));
			double * ck1_Y2 = malloc(partitionLength2 * sizeof(double));
			tally = 0;
			for (int i = 0; i < partitionLength1; i++) {
				ck1_X1[i] = tally;
				tally += hh;
			...
			for (int i = 0; i < partitionLength2; i++)
				ck1 += ck1_Y2[i];
			ck1 = ck1 * hh;
			free(ck1_X1); free(ck1_Y1);
			free(ck1_X2); free(ck1_Y2);
			check1 = ck1;
		...
		*/
		...
		if (check2 <= eps / 3.0) {
			...
			*/
			flag = 1;
			l = 1.0 / m[0];
			double * X_tminusone = malloc(size_XY * sizeof(double));
			for (int i = 0; i < size_XY; i++)
				X_tminusone[i] = X[i] - 1;
			conv2waldpdf(X_tminusone, m[1], s[1], m[2], s[2], Y, h, 0, size_XY);
			free(X_tminusone);
		}
		/* else */
		else {
...
#pragma omp parallel for
			...
			for (int i = 0; i < 2; i++) {
				if (i = 0)
					conv2waldpdf(x, m[1], s[1], m[2], s[2], w[0], h, 0, partitionLength);
				else
...
#pragma omp parallel for
			...
			for (int i = 0; i < 2; i++) {
				if(i == 0)
					conv2waldpdf(x, m[1], s[1], m[2], s[2], w[0], h1, 0, partitionLength);
				else
					waldpdf(x, m[0], s[0], w[1], partitionLength);
			}
