...
double
tailmass (const double m[2], const double s[2], double T2, const double sd[2])
{
...
tailmass (const double m[2], const double s[2], double T2, const double sd[2])
{
  ...
  while (checkerror > 0.0001)
      ...
      if (rtIsNaN (nuplusr) || rtIsNaN (Tu))
	{
      ...
      else if ((hh == 0.0) || ((nuplusr < Tu) && (hh < 0.0))
	       || ((Tu < nuplusr) && (hh > 0.0)))
	{
	  k = b_y->size[0] * b_y->size[1];
	  b_y->size[0] = 1;
      ...
      else if ((rtIsInf (nuplusr) || rtIsInf (Tu))
	       && (rtIsInf (hh) || (nuplusr == Tu)))
	{
	  k = b_y->size[0] * b_y->size[1];
	  b_y->size[0] = 1;
	  b_y->size[1] = 1;
